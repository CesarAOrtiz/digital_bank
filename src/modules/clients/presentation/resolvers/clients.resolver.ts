...
export class ClientsResolver {
  ...
  async createClient(@Args('input') input: CreateClientInput): Promise<ClientGraphqlModel> {
  ...
  async findClients(): Promise<ClientGraphqlModel[]> {
    return (await this.clientsService.findAll()).map((client) => ClientGraphqlMapper.toModel(client));
  }

  @Query(() => ClientGraphqlModel, { name: 'client' })
  async findClient(@Args('id', { type: () => ID }) id: string): Promise<ClientGraphqlModel> {
